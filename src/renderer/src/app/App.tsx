...
import { BoardGridPanel, BoardSwitcherBar, useBoardsEditor, useBoardsQuery } from '@renderer/domains/boards';
...
export function App() {
  const { data: document, isPending, isError, error } = useBoardsQuery();
  ...
  const resolvedActiveBoardId = boards.some((board) => board.id === activeBoardId) ? activeBoardId : boards[0]?.id;
    ...
    <div className="flex h-full">
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="relative min-h-0 flex-1">
          {isPending && <p className="p-6 text-muted">Loading boards…</p>}
